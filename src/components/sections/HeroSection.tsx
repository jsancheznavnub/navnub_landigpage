...
async function fetchSignedUrlForImage(imageKey: string | undefined): Promise<string | null> {
  ...
  if (!backendUrl.startsWith('http://') && !backendUrl.startsWith('https://')) {
    let finalErrorMessage = `fetchSignedUrlForImage (HeroSection): NEXT_PUBLIC_BACKEND_URL ("${backendUrl}") is not a valid absolute URL. It must start with http:// or https://.`;
